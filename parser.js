...
function verifyDistanceMatrixStructure(matrix) {
  ...
  try {
    if (!Array.isArray(matrix)) {
      printErrorAndExit(
        BASE_ERROR_MSG + "Input is not a matrix (not an array)."
      );
    }
    ...
    for (let row of matrix) {
      ...
      if (numOfColumns != numOfRows) {
        printErrorAndExit(
          BASE_ERROR_MSG + "Number of columns and rows don't match"
        );
      }
      ...
      for (let element of row) {
        ...
        if (element < -1) {
          printErrorAndExit(
            BASE_ERROR_MSG + `${element} in the distance matrix is less than -1`
          );
        }
      }
    }
...
function verifyConstraintsStructure(constraintsData) {
  ...
  try {
    if (typeof constraintsData !== "object" || constraintsData === null) {
      printErrorAndExit(
        BASE_ERROR_MSG + "constraints data must be a valid JSON object."
      );
    }
    ...
    if (!("weightCapacity" in constraintsData)) {
      printErrorAndExit(
        BASE_ERROR_MSG + "Missing required key 'weightCapacity'."
      );
    }
    ...
    for (let property in constraintsData) {
      ...
      if (constraintsData[property] < 1) {
        printErrorAndExit(
          BASE_ERROR_MSG +
            `${property} has to be more than greater than or equal to 1 `
        );
      }
    }
  } catch (error) {
  ...
  try {
    ...
    for (const row of packageData) {
      if (typeof row !== "object" || row === null) {
        printErrorAndExit(
          BASE_ERROR_MSG + "Each package must be a valid JSON object."
        );
      }
      ...
      if (!Number.isInteger(row.id) || row.id < 1) {
        printErrorAndExit(
          BASE_ERROR_MSG +
            "id is in invalid format. Must be an integer and greater than 0"
        );
      }
      ...
      if (!Number.isInteger(row.weight) || row.weight < 1) {
        printErrorAndExit(
          BASE_ERROR_MSG +
            "weight is in invalid format. Must be an integer and greater than 0"
        );
      }

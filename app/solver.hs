...
import AirportGraph
import AirplaneType
import AirportGraph (Airport, buildAirportGraph, PathInfo(..))
...
estimateExtraDistance oldPlane newPlane network =
    let oldDest = lastOrOrigin (getPackages oldPlane)
        newDest = last (getPackages newPlane)
        AirportInternal { connections = conns } = network !! oldDest
    in maybe 0 (fromIntegral . distance) (Map.lookup (getDestinationOfPackage newDest) conns)
